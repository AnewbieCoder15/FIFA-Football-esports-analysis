...
Original file is located at
    https://colab.research.google.com/drive/1mk4T-1gYcoGYpQU03uRRO4dE1FV_8QJB
...
df_20= pd.read_csv('/content/players_20.csv',error_bad_lines=False)
...
"""### Calculating BMI 
let's calculate body mass index of each Player 
"""
...
fig.update_layout(title="Scatter Plot (colored by age) year 2020 - Overall vs values in Euros",
xaxis_title='Overall Rating',
yaxis_title='Vlues in Euros' )
fig.show()
...
df_16 = pd.read_csv("/content/players_16.csv", error_bad_lines=False)
df_17 = pd.read_csv("/content/players_17.csv", error_bad_lines=False)
df_18 = pd.read_csv("/content/players_18.csv", error_bad_lines=False)
df_19 = pd.read_csv("/content/players_19.csv", error_bad_lines=False)
...
def playergrowth(name):
    data20=df_20[df_20['short_name'].str.contains(name)]
    data19=df_19[df_19['short_name'].str.contains(name)]
    data18=df_18[df_18['short_name'].str.contains(name)]
    data17=df_17[df_17['short_name'].str.contains(name)]
    data16=df_16[df_16['short_name'].str.contains(name)]
    
    trace20=go.Scatterpolar(
    ...
    data=[trace20,trace19,trace18,trace17,trace16]
    layout= go.Layout(
        polar = dict(
            radialaxis = dict(
                visible = True,
...
fig = px.pie(sample, names='team_position',color_discrete_sequence=px.colors.sequential.Magma_r,
            title='Percentage of Plyers in attacker positions')
fig.show()
...
fig = px.pie(sample, names='team_position',color_discrete_sequence=px.colors.sequential.Magma_r,
            title='Percentage of Plyers in Mid-Fielder positions')
fig.show()
...
fig = px.pie(sample, names='team_position',color_discrete_sequence=px.colors.sequential.Magma_r,
            title='Percentage of Plyers in Defender positions')
fig.show()
...
def top_five(pos,value):
    position="Position_"+str.upper(pos)
    target_players = df_20[(df_20[position]==1) & (df_20['value_eur']<=value)][['short_name','age','overall','value_eur']].head(5)
    return target_players

top_five('lb',28000000)

top_five('st',55000000)
